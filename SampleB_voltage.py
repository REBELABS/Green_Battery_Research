...
##Sample B Voltage analysis
#Carry out the Diffusion Kernel Density of which the Optimum will be arrived at
u_k, omega = diffKDE.KDE(sampleB_volt)

#Carry out the Diffusion KDE at the manual T
u_km, omegam = diffKDE.KDE(sampleB_volt, T = 0.00313)


#Carry out the Diffusion KDE for Optimum KDE * 2
u_km_double, omegam_double = diffKDE.KDE(sampleB_volt, T = 0.00928)

#Superimposed plotting-Ploting of histogram
counts, bins, patchs = plt.hist(sampleB_volt, bins = 9, density = True, alpha = 0.5, label = 'Histogram of Sample B Voltage')

#Plotting the KDEs
#Plotting KDE Optimum
plt.plot(omega, u_k, color = '#1f77b4ff', linestyle = '-', linewidth = 2, label = r"diffKDE, $T^*$")
#Plotting KDE Manual Optimum
plt.plot(omegam, u_km, color = 'green', linestyle = '--', linewidth = 2, label = r"diffKDE, $T^{m} = 0.00313$")
...
#Saving the histo
plt.tight_layout()
plt.savefig('histogram_plotsB.png', dpi = 300, bbox_inches = 'tight')
...
#Carry out the Diffusion Kernel Density of which the Optimum will be arrived at
u_k, omega = diffKDE.KDE(sampleB_curr)

#Carry out the Diffusion KDE at the manual T
u_km, omegam = diffKDE.KDE(sampleB_curr, T = 0.03369)


#Carry out the Diffusion KDE for Optimum KDE * 2
u_km_double, omegam_double = diffKDE.KDE(sampleB_curr, T = 0.15928)

#Superimposed plotting-Ploting of histogram
counts, bins, patchs = plt.hist(sampleB_curr, bins = 'auto', density = True, alpha = 0.5, label = 'Histogram of Sample B Current')

#Plotting the KDEs
#Plotting KDE Optimum
plt.plot(omega, u_k, color = '#1f77b4ff', linestyle = '-', linewidth = 2, label = r"diffKDE, $T^*$")
#Plotting KDE Manual Optimum
plt.plot(omegam, u_km, color = 'green', linestyle = '--', linewidth = 2, label = r"diffKDE, $T^{m} = 0.03369$")
#Plotting for (KDE Optimum *2)
plt.plot(omegam_double, u_km_double, color = 'orange', linestyle = ':', linewidth = 2, label = r"diffKDE, $2{T^{*}}$")
